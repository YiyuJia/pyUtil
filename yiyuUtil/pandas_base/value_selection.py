...
def get_column_via_index(df, index):
    """
...
def get_numbered_columns(df):
    """
    The data frame contains columns with words as well with numbers. We'll do some filtering to extract only the numbered columns:
    :param df:
    :return:
    """
...
def remove_numbered_columns(df):
    """
        The data frame contains columns with words as well with numbers. We'll do some filtering to extract only the stringed columns:
        :param df:
        :return:
    """
    str_list = []  # empty list to contain columns with strings
    for colname, colvalue in df.iteritems():
        if type(colvalue[1]) == str:
            ...
            continue
    # Get to the numeric columns by inversion
    non_num_list = df.columns(str_list)
    return df[non_num_list]