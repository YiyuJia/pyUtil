...
def split_df_by_column_value(df, mask):
    """Split a dataframe into two dataframe.
        Args:
            df (pd.DataFrame): Dataframe.
            mask (bool): mask = df['Sales'] >= 30
        Returns:
            {"mask":df1, "invert_mask":df2}: Two dataframes split from df with mask condition.
        Examples:
            >>> df = pd.DataFrame({'Sales':[10,20,30,40,50], 'A':[3,4,7,6,1]})
            >>> mask = df['Sales'] >= 30
            >>> df_return = split_df_by_column_value(df, mask)

        """
    #could use group by too: df1, df2 = [x for _, x in df.groupby(df['Sales'] < 30)]
    df1 = df[mask]
    df2 = df[~mask]
    return {"mask":df1, "invert_mask":df2}
